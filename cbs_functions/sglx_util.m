...
% Memory map a recording. Returns a memory map
% object m, where m.Data.x holds the memory mapped
% data. For data integrity purposes, the memory 
% mapped file is returned as read-only. 
function m = memmap(meta, binName, path)
    ...
    m = memmapfile(fullfile(path, binName), 'Format', ...
                   {'int16', [nChan, nSamp], 'x'}, 'Repeat', 1);
end % memmap


function dataArray = ReadBinChan(samp0, nSamp, chan, meta, binName, path)
    ...
    fid = fopen(fullfile(path, binName), 'rb');
    fseek(fid, samp0 * 2 * nChan + chan*2, 'bof');
    dataArray = fread(fid, sizeA, 'int16=>double', ...
        2*nChan - 2);
    fclose(fid);
end % ReadBin
...
function dataArray = GainCorrectNI(dataArray, chanList, meta)
    ...
    [MN,MA] = sglx_util.ChannelCountsNI(meta);
    fI2V = sglx_util.Int2Volts(meta);

    for i = 1:length(chanList)
        j = chanList(i);    % index into timepoint
        conv = fI2V / sglx_util.ChanGainNI(j, MN, MA, meta);
...
function dataArray = GainCorrectIM(dataArray, chanList, meta)
    ...
    chans = sglx_util.OriginalChans(meta);
    [APgain,LFgain] = sglx_util.ChanGainsIM(meta);
    nAP = length(APgain);
    nNu = nAP * 2;
